...
- если Lex просит "запушить на GitHub", это означает прямой push в ветку `preview`
- после push в `preview` нужно проверить изменения на `https://amica-ru-test.vercel.app/`
- после подтверждения, что все работает, изменения переносятся из `preview` в `main` отдельным PR
- `main` используется только для подтвержденных изменений, готовых для production
- рабочая схема веток:
  - `main` для production
  - `preview` для стабильного test-стенда на Vercel
  - `feature/...` для обычных задач и отдельных PR при необходимости
...
4. В `Vercel -> amica-ru-test -> Settings -> Domains` привязать `amica-ru-test.vercel.app` к:
   ...
   - ветке `preview`
5. Работать по схеме:
   - `feature/...` -> прямой push в `preview`, если нужен быстрый тест на стабильном стенде
