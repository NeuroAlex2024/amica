...
- сначала изменения доводятся до стабильного состояния локально
- потом тестовый билд выкладывается командой `vercel --prod` в проект `amica-ru-test` в обход GitHub
- тестирование идет на `https://amica-ru-test.vercel.app/`
- если Lex подтверждает, что все работает, изменения пушатся прямо в `main`
- после push в `main` production автоматически обновляется на `https://amica-ru.vercel.app/`
...
### Единый Flow Внедрения Изменений

Текущий рабочий контур деплоя такой:

1. Изменения вносятся и стабилизируются локально.
2. Готовый test-билд выкладывается командой `vercel --prod` в проект `amica-ru-test` в обход GitHub.
3. Проверка идет на `https://amica-ru-test.vercel.app/`.
4. После ручной проверки и подтверждения Lex изменения пушатся прямо в `main`.
5. После push в `main` production обновляется на `https://amica-ru.vercel.app/`.

Это дает:

- один постоянный test URL
- сохранение локальных настроек браузера на test-домене между новыми сборками
